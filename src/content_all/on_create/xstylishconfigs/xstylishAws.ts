...
const textContentToDecoratedHtml = (
    ...
    store: Store,
    colorUuid: boolean
): { uuidColored: boolean, htmlList: string[] } => {
    let ret: string[] = []
    ...
    ret = r(ret, / (?:Chrome|AppleWebKit)\/[.0-9]+/g, decorateX) // ChromeのversionがipAddressとかぶるので先に
    ...
    return { uuidColored, htmlList: ret }
}
...
const walk = (element: HTMLElement, store: Store, recursiveCall: boolean) => {
    ...
    for (let i = 0; i < childNodes.length; i++) {
        ...
        if (nodeName === "#text") {
            ...
            const dom = new DOMParser().parseFromString("<body>" + deco.htmlList.join("") + "</body>", "text/html");
            ...
            if (ele.classList.contains("logs__events__json-string")) {
                const htmlList = textContentToDecoratedHtml(ele, store, false).htmlList
...
const iframeAddedCallback = (iframe: HTMLIFrameElement, colorTask: Promise<{ contentBgColor: string }>) => {
    ...
    if (id === "microConsole-Logs") {
        ...
        style.innerHTML = CloudWatchLogCss
