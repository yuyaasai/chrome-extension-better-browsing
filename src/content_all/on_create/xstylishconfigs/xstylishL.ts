...
const xstylish_EyeFriendly: XstylishConfig = { // eslint-disable-line @typescript-eslint/naming-convention
    name: "",
    targetPage: loc => !textMatches(loc.host, [ // 除外リスト (host:ドメイン名)
        "slideshare",
        "goworkship.com",
        "fern",
        ".salesforce",
        ...
        /(?<!www)\.google\.com$/,
        /([a-z]+\.)?an.e..a..net$/
    ]) && !textMatches(loc.href, [ // 除外リスト (href:URL)
        /console\.(aws\.)?amazon\.com\/[^/]+\/ssh([^a-z]|$)/, // AWS の EC2 Instance Connect
        /\/(www\.)?google\.com\/maps\//, // Google Maps
        /\/webssh\/host/ // Azure の WebSSH
    ]),
    style: `
a,a *,address,b,blockquote,body,button,caption,content,details,div,dd,dl,dt,fieldset,figure,h1,h2,h3,h4,h5,h6,input,
...
/*AWS*/
.cwdb-icon, .GJ-DSRBDLK { font-family: aws-console!important }

/*GoogleCalendar*/
content i, content span { font-family: "Material Icons Extended"!important }
